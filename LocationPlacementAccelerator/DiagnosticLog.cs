...
*
* 1.2: BuildFingerprint inserts the API CallerTag (e.g. "UW") right after
* the LPA prefix when ApiState.Options.CallerTag is set, so per-caller log
* files don't collide with world-gen runs in the same tiny window.
*/
#nullable disable
using BepInEx;
...
namespace LPA
{
	public static class DiagnosticLog
	{
		...
		public static string BuildFingerprint(string versionP = null)
		{
			...
			{
				string tag = ApiState.Options?.CallerTag;
				if (!string.IsNullOrEmpty(tag))
				{
					parts.Append('_');
					parts.Append(SanitizeForFilename(tag));
				}
			}

			parts.Append($"_{enginePrefix}_{modeName}");
		...
		private static string SanitizeForFilename(string sP)
		{
			...
			System.Text.StringBuilder sb = new System.Text.StringBuilder(sP.Length);
